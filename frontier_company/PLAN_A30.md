...
4. **Matched pilot:** if the census has useful mixed groups, run E1 GRPO,
...
5. **Scale decision:** only after matched eval on held-out AIME/AMC/MATH500
...
The larger proposed pairs (4B→8B/14B/32B) are a later feasibility study,
not enabled by this config. Their own offline model assets, full-weight
memory preflight and equal-budget baselines are prerequisites. Four A30s do
not imply each pair will fit or that multiple experiments can run concurrently.
