...
elif [[ "${USE_REMOVE_PADDING:-auto}" == "auto" ]]; then
...
bash "$SCRIPT_DIR/preflight_b200.sh" "$CONFIG_PATH"
echo "[$(date -Is)] attention=$ATTN_IMPLEMENTATION remove_padding=$USE_REMOVE_PADDING"

case "$EXP_ID" in
  E1_grpo_base)
    exec bash "$SCRIPT_DIR/launch_verl.sh" "$CONFIG_PATH" "$EXP_ID" grpo false
    ;;
  E2_opd_base)
    exec bash "$SCRIPT_DIR/launch_verl.sh" "$CONFIG_PATH" "$EXP_ID" token_reward_direct true
    ;;
  S0_hybrid_upstream_sanity|E3_hybrid_static|E4_hybrid_dynamic|E5_hybrid_dynamic_queue)
    ...
    ;;
  *)
    echo "Unsupported experiment: $EXP_ID" >&2
    exit 2
    ;;
esac
